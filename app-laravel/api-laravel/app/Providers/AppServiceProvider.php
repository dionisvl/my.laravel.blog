...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        view()->composer('layout', static function ($view) {
            $view->with(
                'jsParts',
                FrontPart::where('type', 'JS')->where('category_name', env('APP_URL'))->pluck('detail_text')->toArray()
            );
            $view->with(
                'cssParts',
                FrontPart::where('type', 'CSS')->where('category_name', env('APP_URL'))->pluck('detail_text')->toArray()
            );
        });
